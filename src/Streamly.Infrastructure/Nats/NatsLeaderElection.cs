...
public sealed class NatsLeaderElection : ILeaderElection
{
    ...
    private readonly IOptions<StreamlySettings> _runtimeOptions;
    ...
    public async Task<bool> TryAcquireLeadershipAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // Use the KV revision as the epoch. The revision is globally monotonic
            // across all instances (NATS increments it on every write), so epoch
            // always increases regardless of which instance holds leadership or how
            // many terms each instance has had. A local counter (Interlocked.Increment)
            // is per-instance and can produce epochs lower than the previous leader's,
            // causing subscribers to reject valid messages from the new leader.
            var newEpoch = (int)revision;
            Interlocked.Exchange(ref _currentEpoch, newEpoch);
            _isLeader = true;
    ...
    public async Task ReleaseLeadershipAsync(CancellationToken ct = default)
    {
        ...
        {
            // NATS connection was already disposed before leadership release —
            // the KV key will expire via TTL, which is functionally equivalent.
            // This is expected during process shutdown when service stop order
            // disposes the transport before the election service releases the lock.
            _logger.LogDebug(
                "NATS connection disposed before leadership release for stream '{StreamName}' — TTL will handle expiry",
                _streamName);
        }
        catch (Exception ex)
        {
    ...
    private async Task EnsureInitialisedAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Lazy-initializing NATS KV store for leader election (stream '{StreamName}')",
                _streamName);
            ...
            try
            {
                ...
                _logger.LogDebug("Using existing KV bucket '{Bucket}'",
                    _subjectResolver.GetLeaderElectionBucketSubject());
            }
    ...
    private void StartRenewalLoop()
    {
        ...
        _ = Task.Run(async () =>
        {
            var renewalInterval = TimeSpan.FromMilliseconds(
                _runtimeOptions.Value.LeaderElection.HeartbeatIntervalMs);

            _logger.LogDebug(
                "Renewal loop started for stream '{StreamName}' (revision '{Revision}') (interval {Interval}ms)",
                _streamName, capturedRevision, renewalInterval.TotalMilliseconds);
            ...
            while (!ct.IsCancellationRequested && _isLeader)
                try
                {
                    ...
                    // Timeout = one full renewal interval. If NATS is so slow that a
                    // single UpdateAsync cannot complete within that window, the TTL
                    // is at risk of expiring before the next renewal — treat it as a
                    // failure so consecutiveFailures can drive self-demotion.
                    using var renewalTimeout = CancellationTokenSource
                        .CreateLinkedTokenSource(ct);
                    renewalTimeout.CancelAfter(renewalInterval);

                    try
                    {
                        await RenewLeadershipAsync(renewalTimeout.Token).ConfigureAwait(false);
                        consecutiveFailures = 0;
                    }
                    catch (OperationCanceledException) when (
                        !ct.IsCancellationRequested && renewalTimeout.IsCancellationRequested)
                    {
                        // Timeout fired — NATS is too slow, count as failure
                        consecutiveFailures++;
                        _logger.LogWarning(
                            "Renewal timed out after {Interval}ms for stream '{StreamName}' " +
                            "(failure #{Count}/{Max})",
                            (int)renewalInterval.TotalMilliseconds, _streamName,
                            consecutiveFailures, MaxConsecutiveRenewalFailures);
