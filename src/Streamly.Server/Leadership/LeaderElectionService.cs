...
public class LeaderElectionService : ILeaderElectionService
{
    ...
    private async Task ReleaseLeadershipAsync(CancellationToken cancellationToken = default)
    {
        if (!IsLeader)
        {
            _logger.LogDebug(
    ...
    private void OnInfrastructureLeadershipChanged(int newEpoch)
    {
        ...
        // Notify monitoring — CurrentEpoch reads directly from NatsLeaderElection (source of truth)
        _metrics.RecordLeaderStateChange(IsLeader, (int)CurrentEpoch);
