    ...
    where TResponse : class
{
    ...
    private readonly IStreamlyMetricsCollector _metrics = metrics
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (closeReason == null)
        {
            ...
            if (diff.Update == null)
            {
                _metrics.RecordPublishSkipped(requestId);
                _logger.LogTrace(
        ...
        var latestImage = diff?.LatestImage ?? response;
        
        await PublishToTransportAsync(message, requestId, latestImage, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await PublishToTransportAsync(message, requestId, metadata.LatestImage, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            _metrics.RecordPublish(requestId, _streamName, data.Length);
        ...
        {
            _metrics.RecordPublishError(requestId, ex.Message);
            _logger.LogError(ex,
