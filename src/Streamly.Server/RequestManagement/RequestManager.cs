...
internal class RequestManager<TRequest, TResponse> : IRequestManager<TRequest, TResponse>, IAsyncDisposable
{
    ...
    private readonly IResponsePublisher<TRequest, TResponse> _responsePublisher;
    private ILeaderElectionService _leaderElection = null!;
    private readonly IMessageSerializer _serializer;
    private readonly IOptions<StreamlySettings> _options;
    ...
    public RequestManager(
        ...
        IResponsePublisher<TRequest, TResponse> responsePublisher,
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Subscribe to close events from leader
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (_registry.TryAdd(requestId, metadata))
            {
                ...
                // Notify monitoring — new stream, first subscriber
                _metrics.RecordStreamOpened(requestId, _streamName);
                _metrics.RecordRequestJson(requestId, System.Text.Encoding.UTF8.GetString(metadata.SerializedRequest));
                _metrics.RecordSubscriberCountChanged(requestId, 1);
            ...
            else
            {
                ...
                var updatedCountRace = 0;
                _registry.TryUpdate(requestId, m =>
                {
                    m.SubscriberCount++;
                    m.LastUpdateAt = DateTime.UtcNow;
                    updatedCountRace = m.SubscriberCount;
                });
                _metrics.RecordSubscriberCountChanged(requestId, updatedCountRace);
                TrackSubscriberLease(envelope.SubscriberId, requestId);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Notify subscribers via NATS so they receive IsFinal=true and stop retrying.
            // Only the leader publishes — followers skip to avoid duplicate close events.
            if (_leaderElection.IsLeader)
            {
                try
                {
                    await _responsePublisher.CloseAsync(requestId, reason, cancellationToken);
                }
                catch (Exception ex)
                {
            ...
            if (_registry.TryRemove(requestId, out _))
            {
                ...
                // Notify monitoring after the registry entry is gone
                _metrics.RecordStreamClosed(requestId, reason);
            }
    ...
    private async Task OnCloseEventReceivedAsync(byte[] data)
    {
        try
        {
            ...
            // Followers ignore Shutdown close events — the leader is dying and this
            // instance may be elected next. Closing local copies would leave the new
            // leader with an empty registry, forcing all subscribers to cold-reconnect.
            // All other reasons (Unsubscribe, Timeout, Error) must still propagate.
            if (closeEvent.Reason == CloseReason.Shutdown && !_leaderElection.IsLeader)
            {
                _logger.LogInformation(
                    "Ignoring Shutdown close event for '{RequestId}' on '{StreamName}' — " +
                    "keeping request alive for failover",
                    closeEvent.RequestId, _streamName);
                return;
            }
    ...
    private async Task RunBatchSyncLoopAsync(CancellationToken cancellationToken)
    {
        ...
        var interval = TimeSpan.FromMilliseconds(_options.Value.StateSync.BatchSyncIntervalMs);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var metadata = new RequestMetadata<TRequest, TResponse>
            {
                ...
                StreamBehavior = snapshot.StreamBehavior,
            ...
            if (_registry.TryAdd(snapshot.RequestId, metadata))
            {
                ...
                _metrics.RecordStreamOpened(snapshot.RequestId, _streamName);
                _metrics.RecordSubscriberCountChanged(snapshot.RequestId, snapshot.SubscriberCount);

                // Fire-and-forget — handler runs indefinitely for Live streams.
                // Must use metadata.HandlerCts.Token so CloseRequestAsync can cancel it.
                _ = InvokeHandlerOpenedAsync(
                    request,
                    snapshot.RequestId,
                    snapshot.StreamBehavior,
    ...
    {
        var threshold = _options.Value.SubscriberHeartbeatTimeoutMs;
        var now = DateTime.UtcNow;

        _logger.LogDebug(
            "[LeaseCheck] '{StreamName}' — {Count} lease(s), threshold {Threshold}ms",
            _streamName, _subscriberLeases.Count, threshold);
        ...
        foreach (var (subscriberId, lease) in _subscriberLeases)
        {
            ...
            var age = (now - lastHeartbeat).TotalMilliseconds;

            _logger.LogDebug(
                "[LeaseCheck] Subscriber '{SubscriberId}' — last heartbeat {Age:F0}ms ago, " +
                "{Count} request(s), threshold {Threshold}ms",
                subscriberId, age, requestIds.Count, threshold);
    ...
    private async Task RunLeaseCheckLoopAsync(CancellationToken cancellationToken)
    {
        ...
        var interval = TimeSpan.FromMilliseconds(
            _options.Value.SubscriberHeartbeatTimeoutMs / 3.0);
    ...
    private async Task OnUnsubscribeReceivedAsync(byte[] data)
    {
        try
        {
            ...
            {
                m.SubscriberCount = Math.Max(0, m.SubscriberCount - 1);
                updatedCount = m.SubscriberCount;
            ...
            _metrics.RecordSubscriberCountChanged(envelope.RequestId, updatedCount);
    ...
    private void OnLeadershipChanged(object? sender, LeadershipChangedEventArgs e)
    {
        ...
        // If we became leader, sync state to followers then push latest images to subscribers
        if (e.NewState == LeadershipState.Leader)
        {
            ResetAllSubscriberLeases();
            ...
            _ = Task.Run(async () =>
            {
                ...
                await ForcePublishAllLatestImagesAsync(CancellationToken.None);
            });
        }
    }

    private void ResetAllSubscriberLeases()
    {
        var now = DateTime.UtcNow;
        foreach (var (_, lease) in _subscriberLeases)
        {
            lock (lease)
            {
        ...
        _logger.LogInformation(
            "Reset {Count} subscriber lease(s) on '{StreamName}' after leadership gain — " +
            "giving subscribers a fresh heartbeat window",
            _subscriberLeases.Count, _streamName);
    }

    private async Task ForcePublishAllLatestImagesAsync(CancellationToken cancellationToken)
    {
        var requestIds = _registry.GetAllRequestIds().ToArray();

        if (requestIds.Length == 0) return;

        _logger.LogInformation(
            "Force-publishing latest images for {Count} active requests on '{StreamName}' after leadership gain",
            requestIds.Length, _streamName);

        await Task.WhenAll(requestIds.Select(async id =>
        {
            try
            {
                await _responsePublisher.ForcePublishLatestImageAsync(id, cancellationToken);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex,
                    "Failed to force-publish latest image for request '{RequestId}'", id);
            }
        }));
    }
