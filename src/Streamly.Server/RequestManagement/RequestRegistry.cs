    ...
    : IRequestRegistry<TRequest, TResponse>
{
    ...
    public bool TryAdd(string requestId, RequestMetadata<TRequest, TResponse> metadata)
    {
        ...
        if (added)
            _logger.LogDebug("Added request '{RequestId}' to registry (total: {Count})", requestId, _requests.Count);
        else
            _logger.LogDebug("Request '{RequestId}' already exists in registry", requestId);
    ...
    public bool TryUpdate(string requestId, Action<RequestMetadata<TRequest, TResponse>> updater)
    {
        ...
        if (_requests.TryGetValue(requestId, out var metadata))
        {
            ...
            _logger.LogTrace("Updated request '{RequestId}' in registry", requestId);
            return true;
        }

        _logger.LogDebug("Cannot update request '{RequestId}' - not found in registry", requestId);
        return false;
    }
    ...
    public bool TryRemove(string requestId, out RequestMetadata<TRequest, TResponse>? metadata)
    {
        ...
        if (removed)
            _logger.LogDebug("Removed request '{RequestId}' from registry (remaining: {Count})", requestId, _requests.Count);
        else
            _logger.LogDebug("Cannot remove request '{RequestId}' - not found in registry", requestId);

        return removed;
    }

    public IEnumerable<string> GetAllRequestIds() => _requests.Keys.ToList();
    public IEnumerable<RequestMetadata<TRequest, TResponse>> GetAll() => _requests.Values.ToList();
    public bool Contains(string requestId) => !string.IsNullOrWhiteSpace(requestId) && _requests.ContainsKey(requestId);

    public void Clear()
    {
        var count = _requests.Count;
        _requests.Clear();
        _logger.LogInformation("Cleared registry, removed {Count} requests", count);
    }
}
